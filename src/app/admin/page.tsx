...
interface Verification {
  ...
  addrStatus: VerifStatus;
  globalStatus: VerifStatus;
  note: string;
}
...
const s: Record<string, React.CSSProperties> = {
  ...
  loginCard: { background: "#fff", border: "1px solid #D9D9D0", borderRadius: 14, padding: "36px 32px", width: 360, textAlign: "center" as const },
  logo: { fontSize: 28, fontWeight: 800, color: "#2E4A30", marginBottom: 4 },
  logoSub: { fontSize: 13, color: "#6B7B6C", marginBottom: 24 },
  input: { width: "100%", border: "1px solid #D9D9D0", borderRadius: 10, padding: "10px 12px", fontSize: 14, outline: "none", marginBottom: 12, boxSizing: "border-box" as const },
  ...
  sideItem: { display: "block", padding: "10px 20px", color: "rgba(255,255,255,.8)", fontSize: 14, fontWeight: 600, cursor: "pointer", border: "none", background: "transparent", width: "100%", textAlign: "left" as const },
        ...
        {tab === "dashboard" && <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={s.sectionCard}>
              <div style={s.sectionHead}><span style={s.sectionTitle}>Vérifications en attente</span><button style={{ ...s.btnSm("#5A8C5C"), marginRight: 0 }} onClick={() => setTab("verifs")}>Voir tout</button></div>
              {verifs.filter(v => v.globalStatus === "PENDING").slice(0, 3).map(v => (
                <div key={v.id} style={{ padding: "10px 18px", borderBottom: "1px solid #f0f0ee", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={s.badge("PENDING")}>En attente</span>
                </div>
              ))}
              {pendingVerifs === 0 && <div style={{ padding: "16px 18px", fontSize: 13, color: "#6B7B6C" }}>Aucune vérification en attente ✅</div>}
            </div>
            <div style={s.sectionCard}>
              <div style={s.sectionHead}><span style={s.sectionTitle}>Annonces signalées</span><button style={{ ...s.btnSm("#5A8C5C"), marginRight: 0 }} onClick={() => setTab("listings")}>Voir tout</button></div>
              {listings.filter(l => l.reported).map(l => (
                <div key={l.id} style={{ padding: "10px 18px", borderBottom: "1px solid #f0f0ee", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ ...s.badge("REJECTED"), fontSize: 11 }}>⚠️ Signalée</span>
                </div>
              ))}
          ...
          <table style={s.table}>
            <thead>
              <tr>
                ...
                <th style={s.th}>Pièce d'identité</th>
                <th style={s.th}>Adresse</th>
                <th style={s.th}>Statut global</th>
            ...
            <tbody>
              {verifs.map(v => (
                <tr key={v.id}>
                  ...
                  <td style={s.td}><span style={s.badge(v.idStatus)}>{v.idStatus === "APPROVED" ? "✓ Validée" : v.idStatus === "REJECTED" ? "✗ Rejetée" : "En attente"}</span></td>
                  <td style={s.td}><span style={s.badge(v.addrStatus)}>{v.addrStatus === "APPROVED" ? "✓ Validée" : v.addrStatus === "REJECTED" ? "✗ Rejetée" : "En attente"}</span></td>
                  <td style={s.td}><span style={s.badge(v.globalStatus)}>{v.globalStatus === "APPROVED" ? "✓ Vérifié" : v.globalStatus === "REJECTED" ? "✗ Rejeté" : "⏳ En attente"}</span></td>
                  <td style={s.td}>
                    {v.globalStatus === "PENDING" && <>
                      <button style={s.btnSm("#5A8C5C")} onClick={() => approveVerif(v.id)}>✓ Approuver</button>
                      <button style={s.btnSm("#c0392b")} onClick={() => { setModal(v); setRejectNote(""); }}>✗ Rejeter</button>
                    </>}
                    {v.globalStatus === "APPROVED" && <span style={{ fontSize: 12, color: "#5A8C5C", fontWeight: 600 }}>Dossier approuvé</span>}
              ...
              {listings.map(l => (
                <tr key={l.id} style={{ opacity: l.status === "DELETED" ? .5 : 1 }}>
                  <td style={s.td}>
                    ...
                    <div style={{ fontSize: 12, color: "#6B7B6C" }}>{l.createdAt}{l.reported && <span style={{ color: "#c0392b", marginLeft: 6 }}>⚠️ Signalée</span>}</div>
                  </td>
                  <td style={s.td}>{l.seller}</td>
                  <td style={s.td}>CHF {l.price}.–</td>
                  <td style={s.td}>{l.city}</td>
                  <td style={s.td}>{l.category}</td>
                  <td style={s.td}><span style={s.badge(l.status)}>{l.status === "ACTIVE" ? "Active" : "Supprimée"}</span></td>
                  <td style={s.td}>
                    {l.status === "ACTIVE"
                      ? <button style={s.btnSm("#c0392b")} onClick={() => deleteListing(l.id)}>🗑 Supprimer</button>
          ...
          <table style={s.table}>
            <thead>
              <tr>
                <th style={s.th}>Utilisateur</th>
                <th style={s.th}>Ville</th>
                <th style={s.th}>Membre depuis</th>
                <th style={s.th}>Annonces</th>
                <th style={s.th}>Vérification</th>
                <th style={s.th}>Statut</th>
                <th style={s.th}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id}>
                  ...
                  <td style={s.td}><span style={s.badge(u.verified ? "APPROVED" : "PENDING")}>{u.verified ? "✓ Vérifié" : "Non vérifié"}</span></td>
                  <td style={s.td}><span style={s.badge(u.status)}>{u.status === "ACTIVE" ? "Actif" : "Suspendu"}</span></td>
                  <td style={s.td}>
                    <button style={s.btnSm(u.status === "ACTIVE" ? "#e67e22" : "#5A8C5C")} onClick={() => toggleUser(u.id)}>
      ...
      {modal && (
        <div style={s.modalOverlay} onClick={() => setModal(null)}>
          <div style={s.modalCard} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 13, color: "#6B7B6C", marginBottom: 16 }}>Dossier de <strong>{modal.name}</strong>. Une note sera ajoutée au dossier.</p>
            <textarea
              style={{ ...s.input, minHeight: 80, resize: "vertical", marginBottom: 16 }}
              placeholder="Raison du rejet (ex: document illisible, adresse non conforme…)"
              value={rejectNote}
              onChange={e => setRejectNote(e.target.value)}
            />
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button style={{ ...s.btnSm("#6B7B6C"), padding: "8px 16px", fontSize: 13 }} onClick={() => setModal(null)}>Annuler</button>
              <button style={{ ...s.btnSm("#c0392b"), padding: "8px 16px", fontSize: 13 }} onClick={() => rejectVerif(modal.id, rejectNote)}>Confirmer le rejet</button>
            </div>
          </div>
        </div>
      )}
